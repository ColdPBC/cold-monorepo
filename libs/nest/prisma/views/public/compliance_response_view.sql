      ...
      (
        (
          (
            ...
            JOIN compliance_section_groups csg ON (
              (
                csg.compliance_definition_name = oc.compliance_definition_name
              )
            )
          )
          JOIN compliance_sections cs ON ((csg.id = cs.compliance_section_group_id))
        )
        JOIN compliance_questions cq ON ((cs.id = cq.compliance_section_id))
      )
