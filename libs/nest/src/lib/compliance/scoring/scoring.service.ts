...
export class ScoringService extends BaseWorker {
	...
	async scoreQuestion(
		...
		if (Array.isArray(question.compliance_responses)) {
			...
			if (question.rubric?.max_score) {
				...
				maxScore = this.getRubricTopScore(question.rubric);
			}

			// Set the score for the question
			question.score = score;
			question.ai_score = aiScore;
			question.max_score = maxScore || this.getRubricTopScore(question.rubric);
		} else {
			// If the question has no responses, set the ai_score & score to 0
			question.score = 0;
			question.ai_score = 0;
			question.max_score = question.rubric?.max_score || this.getRubricTopScore(question.rubric);
		}
	...
	private getRubricTopScore(rubric: any) {
		let maxScore = 0;
		if (rubric) {
