...
export class RolesGuard extends BaseWorker implements CanActivate {
	constructor(private reflector: Reflector, readonly prisma: PrismaService /*, private readonly cache: CacheService*/) {
		super('RolesGuard');
	}
	...
	async resolveRequest(request: any, user: IAuthenticatedUser, roles: Array<string>) {
		...
		if (!orgId) {
			...
			if ((request?.params?.nameOrId || request?.params?.orgId) && !request?.params?.nameOrId?.includes('org_') && !request?.params?.orgId?.includes('org_')) {
				...
				}); //(await this.cache.get('organizations')) as Organizations[];

				if (!org) {
					this.logger.warn(`Unable to find org by name ${request?.params?.nameOrId | request?.params?.orgId}`);
				} else {
					//
