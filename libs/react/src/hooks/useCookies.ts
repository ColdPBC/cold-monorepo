...
export const useCookies = () => {
    ...
    const setCookieData = (user: any, token: string, expires = new Date(1000 * 60 * 60 * 24)) => {
        // Store the user and token in a cookie
        cookie.set('coldpbc', JSON.stringify({ accessToken: token, expires, user }), {path: '/', sameSite: 'strict', secure: true});
    }
