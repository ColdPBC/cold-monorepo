...
export const useAuth0Wrapper = () => {
	...
	const userData = useSWR<User, any, any>(auth0Context.user && auth0Context.isAuthenticated ? [`/members/${auth0Context.user.email}`, 'GET'] : null, axiosFetcher);
	...
	if (auth0Context.user) {
		orgId = impersonatingOrg ? impersonatingOrg.id : auth0Context.user.coldclimate_claims.org_id;
	}
