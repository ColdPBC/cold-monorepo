...
import { ColdIcon, ColdLeftArrowIcon, ComplianceManagerOverview, ComplianceManagerPreview, ErrorFallback, Spinner, Tabs } from '@coldpbc/components';
		...
		if (compliance) {
			...
			if (files.data && files.data.length > 0) {
				...
				if (currentAIStatus?.data && currentAIStatus.data.length > 0) {
					logBrowser(`Setting ${name} compliance manager status to started AI`, 'info', {
						name,
						currentAIStatus,
					});
					...
					if (statuses && statuses.length > 0) {
						...
						if (recentStatus.type === 'user_submitted') {
							logBrowser(`Setting ${name} compliance manager status to submitted`, 'info', {
								name,
								recentStatus,
							});
	...
	return (
		<ColdComplianceManagerContext.Provider
			...
			<div className={'flex flex-col w-full gap-[48px] justify-center relative mb-[40px]'}>
				...
				<Tabs tabs={tabs} className={'flex flex-col w-full gap-[48px] px-[64px] justify-items-center items-center'} defaultTab={'Overview'} />
			</div>
		</ColdComplianceManagerContext.Provider>
	);
};
