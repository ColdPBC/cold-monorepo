...
import { DataGridCellHoverPopover, ErrorFallback, MuiDataGrid, Spinner, SustainabilityAttributeColumnList } from '@coldpbc/components';
import { GridColDef, GridToolbarColumnsButton, GridToolbarContainer, GridToolbarExport, GridToolbarQuickFilter, GridValidRowModel } from '@mui/x-data-grid';
...
const _MaterialsDataGrid = () => {
	...
	const uniqTier1Suppliers = uniq(materials.map(material => material.productMaterials.map(productMaterial => productMaterial.product.organizationFacility.name)));
	...
	materials.forEach(material => {
		...
		const allRelatedAttributeAssurances = [...material.attributeAssurances, ...extraAttributes];
	...
	return (
		<div className={'w-full'}>
			<MuiDataGrid rows={rows} columns={columns} slots={{ toolbar: getToolbar }} columnHeaderHeight={55} rowHeight={114} />
		</div>
	);
};
