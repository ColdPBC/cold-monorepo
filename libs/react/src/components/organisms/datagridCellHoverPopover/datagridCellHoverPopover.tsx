...
export const DataGridCellHoverPopover = (props: { params: GridRenderCellParams<any, any, any, GridTreeNodeWithRender>; color?: string }) => {
	const { params, color = HexColors.primary.DEFAULT } = props;
	const values = (params.value as string[]).sort();
	const [valuesToDisplay, setValuesToDisplay] = React.useState<string[]>(values);
	...
	return (
		<div className={'h-full w-full flex items-center justify-start text-body text-tc-primary gap-[10px]'} ref={containerRef}>
			...
			<div ref={divRef} className={'w-auto flex items-center gap-[10px] invisible whitespace-nowrap'}>
