...
export const ComplianceOverviewCard = (props: ComplianceOverviewCardProps) => {
  ...
  const getCtaButton = () => {
    if (onOverviewPage) {
      ...
      // on the detail page. if all the questions are answered, show the Edit Answers button
      if (isOverview) {
        return null;
      } else {
        // if there are no questions answered, show the Start Survey button
        if (complianceData.answeredQuestions === 0 && complianceData.aiAnsweredQuestions === 0) {
          return <BaseButton label={'Start Survey'} variant={ButtonTypes.primary} size={GlobalSizes.small} onClick={ctaOnClick} />;
        } else {
          if (complianceData.answeredQuestions === complianceData.totalQuestions) {
            return <BaseButton label={'Edit Answers'} variant={ButtonTypes.primary} size={GlobalSizes.small} onClick={ctaOnClick} />;
          } else {
