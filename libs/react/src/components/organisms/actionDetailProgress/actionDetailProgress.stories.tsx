...
export const ReadyToExecute: Story = {
  ...
  play: async ({ canvasElement, args, step }) => {
    ...
    await step('Click on Add Steward', async () => {
      ...
      await expect(stewardButtonsAfter.length).toBeLessThan(
        stewardButtonsInitial.length,
      );
    });
    ...
    await step('Click on Complete', async () => {
      ...
      await expect(checkboxesAfter.length).toBeGreaterThan(
        checkboxesInitial.length,
      );
      // check canvas for 'number of steps that are completed/total steps completed' text
      await canvas.getByText(
        `${checkboxesAfter.length}/${steps.length} Completed`,
      );
    });
