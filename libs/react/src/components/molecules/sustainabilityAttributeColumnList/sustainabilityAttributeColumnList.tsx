...
export const sustainabilityAttributeSortFn = (a: SustainabilityAttributeWithStatus, b: SustainabilityAttributeWithStatus): number => {
...
const _SustainabilityAttributeColumnList: React.FC<SustainabilityAttributeColumnListProps> = ({ sustainabilityAttributes }) => {
	...
	const sortedSustainabilityAttributesWithStatus: SustainabilityAttributeWithStatus[] = sustainabilityAttributes
		.map(sustainabilityAttribute => {
	...
	return (
		<div
			ref={containerRef}
			className="px-4 py-4 flex items-start w-full"
			style={{
				gap: `${CARD_GAP}px`,
				minWidth: `${MIN_VISIBLE_CARDS * TOTAL_CARD_WIDTH - CARD_GAP + CONTAINER_PADDING_X}px`,
			}}>
			{visibleAttributes.map(sustainabilityAttribute => (
				<SustainabilityAttributeLogoWithStatus sustainabilityAttribute={sustainabilityAttribute} key={sustainabilityAttribute.id} />
			))}
