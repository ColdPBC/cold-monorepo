...
export interface FootprintOverviewHorizontalDetailProps
  extends FootprintOverviewDetail {
  leftAlign: boolean; // if it's not left aligned, it's right aligned
}

export function FootprintOverviewHorizontalDetail(
  props: PropsWithChildren<FootprintOverviewHorizontalDetailProps>,
) {
  return (
    <>
      {props.leftAlign && (
        <FootprintOverviewHorizontalDetailRectangle color={props.color} />
      )}
      <div
        className={
          'flex flex-col justify-center gap-1 ' +
          (props.leftAlign ? 'items-start' : 'items-end')
        }
      >
        <div className={'text-body font-bold text-tc-primary'}>
          {props.title}
        </div>
