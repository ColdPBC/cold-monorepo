...
export function FootprintOverviewChart(
  ...
  const isEmptyFootprintData =
    !isLoading &&
    !data.subcategories?.some((subcategory: any) =>
      subcategory.activities?.some((activity: any) =>
        activity.footprint?.some(
  ...
  forEach(data.subcategories?.slice(0, MAX_CATEGORIES), (subcategory) => {
    ...
    forEach(subcategory.activities, (activity) => {
      if (activity.footprint) {
        const footprint = find(activity.footprint, {
          period: period,
      ...
      beforeDraw(chart: ChartJS): boolean | void {
        chart.getDatasetMeta(0).data.forEach((slice, index) => {
          ...
          if (chartActiveSegment && index === chartActiveSegment.index) {
            ...
            slice.options.backgroundColor =
              hoverColorArray[index === 0 ? 0 : index / 2];
          } else {
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
