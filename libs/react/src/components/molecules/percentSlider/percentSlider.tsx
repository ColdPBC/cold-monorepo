    ...
    return (
        <div className={"w-full space-y-2 text-tc-primary"}>
            <div className={"flex w-full"}>
                <span className={"text-xs not-italic font-medium"}>
                    Enter %
                </span>
            </div>
            <div className={"flex w-full space-x-[24px] justify-between"}>
                <NumericFormat value={getInputValue()} onValueChange={onInputValueChange}
                               className={"w-[56px] h-[56px]  rounded-lg py-4 text-sm text-center font-bold not-italic bg-transparent border border-bgc-accent focus:border focus:border-bgc-accent focus:ring-0"}
                               allowNegative={false} />
