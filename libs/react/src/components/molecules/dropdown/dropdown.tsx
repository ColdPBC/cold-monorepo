	...
	return (
		<Listbox
			...
			<div className={twMerge('relative w-auto mt-1', props.containerClassName)}>
				<Listbox.Button className="relative w-full flex flex-row justify-between cursor-default rounded-lg bg-transparent p-[16px] text-left text-tc-secondary text-body border-[1.5px] border-bgc-accent active:border-primary">
				...
				<Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
					<Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-lg bg-bgc-elevated py-1 text-body text-tc-secondary border-[1.5px] border-bgc-accent z-10">
						{props.options.map((option, index) => (
							<Listbox.Option key={index} className={({ active }) => `relative cursor-pointer select-none p-[16px] border-[0] text-left hover:bg-bgc-accent`} value={option}>
