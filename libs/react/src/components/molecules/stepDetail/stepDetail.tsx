  ...
  return (
    <Disclosure as="div">
      {({ open }) => (
        <div
          className={
            'border-[1px] border-bgc-accent rounded-lg bg-bgc-elevated text-tc-primary'
          }
        >
          <Disclosure.Button
            className={
              'z-0 flex w-full p-[16px] space-x-[16px] justify-between items-center'
            }
            onClick={(event) => {
              if (
                assigneeSelectorRef.current?.contains(event.target as Node) ||
                stepDetailsCheckboxRef.current?.contains(event.target as Node)
              ) {
