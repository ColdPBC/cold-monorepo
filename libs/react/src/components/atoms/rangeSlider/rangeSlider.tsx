...
export const RangeSlider = ({ min, max, defaultValue, label, inputName, onChange, title }: RangeSliderProps) => {
	const getRangeValue = () => {
	...
	return (
		<div>
			{label && <label className="block mb-2 text-sm font-medium text-cold-jetBlack dark:text-white">{label}</label>}
			<Range
				min={min}
				max={max}
				...
				renderThumb={({ props, isDragged }) => (
					<div
						...
						<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
							<circle cx="12" cy="12" r="10" fill={HexColors.gray['120']} stroke={HexColors.primary['300']} strokeWidth="4" />
						</svg>
					</div>
				)}
			/>
