...
export function convert(hex: string | DefaultHexColors, addAlpha:boolean) {
  ...
  if(hsla.a) {
    set(formats.hsla, 'hslaString', toString( {
      hsla: {
        h: hsla.h,
        s: hsla.s,
        l: hsla.l,
...
export function generateVariants(hexColor: string, addAlpha = false): any {
  ...
  const lightValues = [.10, .20, .30, .40, .50, .60, .70, .80, .90, 1, 1.10, 1.20, 1.30, 1.40, 1.50, 1.60, 1.70, 1.80, 1.90];

  // Generate variants by adjusting the lightness property of the color
  lightValues.map((value, idx) => {
    const converted = { rgba: hslaToRGBA( hsla.h, hsla.s, hsla.l * value, hsla.a ) }
