...
export const ColdMQTTProvider = ({ children }: PropsWithChildren) => {
  ...
  useEffect(() => {
    ...
    const connectToIOT = async () => {
      ...
      const url = `wss://a2r4jtij2021gz-ats.iot.us-east-1.amazonaws.com:443/mqtt?x-auth0-domain=${auth0_domain}&x-amz-customauthorizer-name=${authorizer}&x-cold-org=${org_id}&x-cold-env=staging&token=${token}`;
      const account_id = user?.email;
      const subscription_topic = `ui/staging/${org_id}/${account_id}`;
