...
export class Material {
	...
	@Index({ name: 'materials_name_idx1' })
	@Property({ type: 'text' })
	name!: string;
	...
	async afterCreate(params: CreateOrUpdateHookParams<Material, OrgContext>) {
		if (!this.sidecar) {
			this.sidecar = new MaterialHooks();
		}
	...
	async afterUpdate(params: CreateOrUpdateHookParams<Material, OrgContext>) {
		if (!this.sidecar) {
			this.sidecar = new MaterialHooks();
		}
	...
	async afterDelete(params: DeleteHookParams<Material, OrgContext>) {
		if (!this.sidecar) {
			this.sidecar = new MaterialHooks();
		}
