...
export const sgs = z.object({
  ...
  testing_company: z
    .object({
      ...
      address_line_1: z.string().describe('If the content contains an applicant address, place the street address here here'),
      address_line_2: z.string().describe('If the content contains an applicant address, place the unit/suite or care of information here'),
      city: z.string().describe('If the content contains an applicant address, place the city here'),
      state_province: z.string().describe('If the content contains an applicant address, place the state or province here'),
      postal_code: z.string().describe('If the content contains an applicant address, place the postal code here'),
      country: z.string().describe('If the content contains an applicant address, place the country here'),
      metadata: z.object({
        email: z.string().describe('If the content contains an applicant email, place it here'),
        phone: z.string().describe('If the content contains an applicant phone number, place it here'),
        website: z.string().describe('If the content contains an applicant website, place it here'),
      }),
    })
    .describe('If the content is a test document, attempt to extract data for testing company information'),
  test_date: z.string().describe('If the content contains a test date, place it here'),
  test_number: z.string().describe('If the content contains a test number, place it here'),
  summary: summary,
  supplier: z.object({
    name: z.string().describe('If the content contains an applicant, place it here'),
