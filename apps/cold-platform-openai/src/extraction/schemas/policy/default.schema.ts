...
export const defaultPolicySchema = z.object({
  ...
  effective_end_date: effective_end_date.describe(
    'If the document has an expiration or end date, place it here as an ISO 8601 formatted date.  If not, and there is an effective start date, then insert a date 1 year from the effective start date, otherwise leave blank',
  ),
  version: version,
});
