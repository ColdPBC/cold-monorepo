...
export const meters_schema = z.array(
  z
    .object({
      id: z.string(),
      type: z.enum(['gas', 'electricity']).optional(),
...
export const bill_parsed_schema = z
  .object({
    ...
    billing_period_from: z.string().datetime({ offset: false }).pipe(z.coerce.date()),
    billing_period_to: z.string().datetime({ offset: false }).pipe(z.coerce.date()),
    utility: z.string(),
    ...
    gas_consumption: z.number().nullable(),
