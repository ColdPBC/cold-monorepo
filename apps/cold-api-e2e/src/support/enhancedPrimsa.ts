...
export const enhancePrisma = <TPrismaClientCtor extends Constructor>(client: TPrismaClientCtor): EnhancedPrismaClientConstructor<TPrismaClientCtor> => {
      ...
      if (!global._blitz_prismaClient) {
        ...
        client.$reset = async function reset() {
          if (process.env.NODE_ENV === 'production' || process.env.NODE_ENV === 'production') {
            throw new Error(
              "You are calling db.$reset() in an important environment. We think you probably didn't mean to do that, so we are throwing this error instead of destroying your life's work.",
            );
          }
