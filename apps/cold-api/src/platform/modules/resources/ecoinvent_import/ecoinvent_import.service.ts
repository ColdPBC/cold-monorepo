...
export class EcoinventImportService extends BaseWorker {
	...
	async importCsv(req: any, bucketName = 'cold-ecoinvent-data', csvFileKey = 'ecoinvent_ecospold2_3_11_cutoff/FilenameToActivityLookup.csv'): Promise<void> {
		...
		try {
			...
			this.logger.error(`Error reading CSV file content from S3: ${csvFileKey}`, { bucket: bucketName, key: csvFileKey, user: req.user, error: { ...e } });
		}
	}

	async queueImportJobs(req: any, location?: string, reprocess?: boolean, ignoreCached?: boolean): Promise<any> {
		const filter: any[] = [];
