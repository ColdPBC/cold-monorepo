...
import { allRoles, HttpExceptionFilter, IRequest, JwtAuthGuard, Roles, RolesGuard } from '@coldpbc/nest';
...
export class MaterialsController {
  ...
  @Post()
  create(@Req() req: IRequest, @Param('orgId') orgId: string, @Body() createMaterialDto: any) {
    return this.materialsService.create(req, createMaterialDto);
  }

  @Post('/:id/supplier/:supplierId')
  @Roles(...allRoles)
  createMaterialSupplier(@Req() req: IRequest, @Param('orgId') orgId: string, @Param('id') id: string, @Param('supplierId') supplierId: string) {
    return this.materialsService.createSupplierMaterial(req, id, supplierId);
  }

  @Delete('/:id/supplier/:supplierId')
  @Roles(...allRoles)
  delete(@Req() req: IRequest, @Param('orgId') orgId: string, @Param('id') id: string, @Param('supplierId') supplierId: string) {
    return this.materialsService.deleteSupplierMaterial(req, id, supplierId);
  }

  @Post('batch')
  @Roles(...allRoles)
  createMany(@Req() req: IRequest, @Param('orgId') orgId: string, @Body() createMaterialDto: any) {
  ...
  @Patch(':id')
  @Roles(...allRoles)
  update(@Req() req: IRequest, @Param('orgId') orgId: string, @Param('id') id: string, @Body() updateMaterialDto: any) {
    return this.materialsService.update(req, id, updateMaterialDto);
  }

  @Delete(':id')
  @Roles(...allRoles)
  remove(@Req() req: IRequest, @Param('orgId') orgId: string, @Param('id') id: string) {
