...
export class ScoringService extends BaseWorker {
   ...
   * @param survey
   */
  scoreAssesment(assessment: any): Promise<any> {
    // Iterate over each section in the survey
    Object.keys(assessment.definition.sections).forEach(sectionKey => {
      // Get the follow-up questions for the current section
      const followUps = assessment.definition.sections[sectionKey].follow_up;
  ...
  private getTopScore(question: compliance_questions) {
    let maxScore = 0;
    const rubric = question.rubric as any;
    Object.keys(rubric?.score_map).forEach(value => (parseInt(rubric.score_map[value]) > maxScore ? (maxScore = parseInt(rubric.score_map[value])) : maxScore));
    return maxScore;
  }
}
