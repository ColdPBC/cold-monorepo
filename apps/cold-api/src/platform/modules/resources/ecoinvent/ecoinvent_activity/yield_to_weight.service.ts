import { ProductMaterialForWeightCalculation, Area, Count, Length, Weight, WeightFactorUnits } from './weight.types';
...
export const getCalculatedWeight = (productMaterial: ProductMaterialForWeightCalculation): CalculatedWeightResult => {
	...
	const unitOfMeasure = productMaterial.unit_of_measure;
	...
	if (unitOfMeasure === 'pcs') {
		const weightFactorOverride = productMaterial.material.weight_factor_unit_of_measure === WeightFactorUnits.KG_PER_PCS ? productMaterial.material.weight_factor : undefined;
		weightFactor = weightFactorOverride || GLOBAL_PIECES_WEIGHT_FACTOR;
	} else {
		const weightFactorOverride = productMaterial.material.weight_factor_unit_of_measure === WeightFactorUnits.KG_PER_M2 ? productMaterial.material.weight_factor : undefined;
		weightFactor = weightFactorOverride || productMaterial.material.material_classification?.weight_factor;
	}
	...
	switch (unitOfMeasure) {
		...
		case Length.yd:
		case Length.m:
			return calculateWeightFromLength(productMaterial.yield, unitOfMeasure, productMaterial.material.width, productMaterial.material.width_unit_of_measure, weightFactor);
		default:
